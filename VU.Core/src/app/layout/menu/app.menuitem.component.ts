          ...
          <ng-container>
              <a [attr.href]="item.url" (click)="itemClick($event)" *ngIf="(!item.routerLink || item.items) && item.visible !== false"
                ...
                <i [ngClass]="item.icon" class="layout-menuitem-icon"></i>
                  ...
                  <i class="pi pi-fw pi-angle-right layout-submenu-toggler layout-menuitem-icon" *ngIf="item.items"></i>
              </a>
              <a (click)="itemClick($event)" (mouseenter)="onMouseEnter()" *ngIf="(item.routerLink && !item.items) && item.visible !== false"
                  ...
                  <span class="menuitem-text">{{item.label}}</span>
                  <i class="pi pi-fw pi-angle-right layout-submenu-toggler" *ngIf="item.items"></i>
                  <!-- <i [ngClass]="item.icon" class="layout-menuitem-icon"></i> -->
              </a>
              ...
              (active ? 'visibleAnimated' : 'hiddenAnimated')">
                  <ng-template ngFor let-child let-i="index" [ngForOf]="item.items">
                      <li app-menuitem [item]="child" [index]="i" [parentKey]="key" [class]="child.badgeClass" *ngIf="child.isShow"></li>
                  </ng-template>
              </ul>
          </ng-container>
      `,
