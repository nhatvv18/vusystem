...
export class ServiceProxyBase {
	 ...
	 */
	public getHeaderToken(cookie: any = AppConsts.authorization.accessToken) {
	...
	protected requestPostFileUtilTemp(body: { file: null }, url: String): Observable<any> {
		let url_ = `https://api-dev.epicpartner.vn` + url;
	...
	protected requestPutFileUtil(body: { file: null }, url: String): Observable<any> {
		let url_ = this.baseUrl + url;
		url_ = url_.replace(/[?&]$/, "");
		let formData: FormData = new FormData();
